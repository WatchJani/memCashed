...
func (c *Config) Workers() *hash_table.Engine {
	numberOfWorkers := len(c.DefaultSlab)
	if numberOfWorkers == IntDefaultValue {
		numberOfWorkers = DefaultNumberOfWorkers
	}

	return hash_table.NewEngine(uint32(numberOfWorkers))
}
